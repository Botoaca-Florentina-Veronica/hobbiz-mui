    ...
    <ThemedView style={[styles.container, { backgroundColor: tokens.colors.bg }]}>      
      <ScrollView style={Platform.OS === 'web' ? ({ height: '100vh' } as any) : { flex: 1 }} contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.sectionCard, { backgroundColor: surface }]}>          
          ...
          <ThemedText style={[styles.paragraph, { color: bodyColor }]}>{
            isEn
              ? 'Posting listings or any other content (including in messages, images, reviews, or profiles) that includes counterfeit or pirated goods, illegal substances, weapons and ammunition, protected or illegally obtained animals, illegal or immoral services, nudity or sexually explicit content, depictions of explicit real-world violence (outside of a journalistic or educational context), hate speech, or materials infringing copyright is strictly prohibited. We actively monitor content using automated algorithms and human moderation.'
              : isEs
                ? 'Esta estrictamente prohibido publicar anuncios o cualquier otro contenido (incluido en mensajes, imagenes, resenas o perfiles) que incluya productos falsificados o piratas, sustancias ilegales, armas y municiones, animales protegidos u obtenidos ilegalmente, servicios ilegales o inmorales, desnudez o contenido sexualmente explicito, representaciones de violencia explicita del mundo real (fuera de un contexto periodistico o educativo), discurso de odio, o materiales que infrinjan derechos de autor. Supervisamos el contenido con algoritmos automáticos y moderacion humana.'
                : 'Este strict interzisă publicarea de anunțuri sau a oricărui alt conținut (inclusiv în mesaje, imagini, recenzii sau profil) care include produse contrafăcute sau piratate, substanțe interzise prin lege, arme și muniții, animale protejate sau obținute ilegal, servicii ilegale sau imorale, nuditate ori conținut cu caracter sexual explicit, reprezentări de violență explicită din lumea reală (în afara unui context jurnalistic sau educațional), discurs instigator la ură sau materiale ce încalcă drepturile de autor. Monitorizăm activ conținutul prin algoritmi automatizați și moderare umană.'
          }</ThemedText>
        ...
        <View style={[styles.sectionCard, { backgroundColor: surface }]}>          
          ...
          <ThemedText style={[styles.paragraph, { color: bodyColor }]}>{
            isEn
              ? 'We use the Expo push notification service to deliver in-app notifications, which may relay messages to Android devices via Firebase Cloud Messaging. This provider may process certain data on our behalf (for example: push tokens). Please review its privacy policy for details. You can disable notifications anytime via the app settings.'
              : isEs
                ? 'Usamos el servicio de notificaciones push de Expo para entregar notificaciones en la app, que puede retransmitir mensajes a dispositivos Android mediante Firebase Cloud Messaging. Este proveedor puede procesar ciertos datos en nuestro nombre (por ejemplo: tokens push). Revisa su politica de privacidad para mas detalles. Puedes desactivar las notificaciones en cualquier momento desde los ajustes de la app.'
                : 'Folosim serviciul de notificări push Expo pentru livrarea notificărilor în aplicație, care poate retransmite mesajele către dispozitivele Android prin Firebase Cloud Messaging. Acest furnizor poate procesa anumite date în numele nostru (de exemplu: token-uri push). Vă rugăm să consultați politica sa de confidențialitate pentru detalii. Puteți dezactiva notificările oricând din setările aplicației.'
          }</ThemedText>
          <ThemedText style={[styles.paragraph, { color: bodyColor }]}>{
            isEn
